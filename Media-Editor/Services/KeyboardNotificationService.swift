    ...
    init() {
        keyboardWillShowNotificationPublisher = NotificationCenter.Publisher(
            ...
            .tryMap { userInfo in
                ...
                guard let animationCurveValue = userInfo[UIResponder.keyboardAnimationCurveUserInfoKey] as? Int else {
            ...
            .tryCompactMap { keyboardRect, animationCurveValue, animationDuration in
                guard let curve = UIView.AnimationCurve(rawValue: animationCurveValue) else { throw KeyboardError.invalidCurveRawValue(rawValue: animationCurveValue) }
                return (animationDuration: curve.convertToAnimation(withDuration: animationDuration), keyboardHeight: keyboardRect.height)
            }
            .mapError {
